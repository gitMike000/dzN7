...
const wchar_t separ =
    *reinterpret_cast<const wchar_t *>(&fs::path::preferred_separator);
...
int main(int argc, const char* const argv[])
{
    ...
    svr.Get("/get", [&](const httplib::Request &req, httplib::Response &res) {
        ...
        for (auto param : req.params)
        {
            ...
            if (param.first == "beans") { fict = 1; size_fict = std::stoi(param.second); }
            if (param.first == "size") { fict = 2; size_fict = std::stoi(param.second); }
        }
